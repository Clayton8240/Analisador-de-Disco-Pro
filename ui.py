...
# Importa a lógica de análise e os utilitários
import analysis
import utils
...
class FinalDiskAnalyzerApp(tk.Tk):
    ...
    def create_filter_panel(self, parent_tab):
        ...
        ttk.Label(left_frame, text="Nome/Caminho:").pack(anchor='w')
        ...
        ttk.Label(right_frame, text="Categorias:").pack(anchor='w')
        ...
        for category in self.category_map.keys():
            var = tk.BooleanVar()
            cb = ttk.Checkbutton(types_frame, text=category, variable=var)
        ...
        self.btn_apply_filters = ttk.Button(action_frame, text="Aplicar Filtros", command=self.apply_filters)
        self.btn_apply_filters.pack(side='left')
        self.btn_clear_filters = ttk.Button(action_frame, text="Limpar", command=self.clear_filters)
    ...
    def create_file_list_table(self, parent_tab):
        ...
        self.files_tree.column("Nome", width=250)
        self.files_tree.column("Tamanho (MB)", anchor='e', width=120)
        self.files_tree.column("Data de Modificação", width=150)
        self.files_tree.column("Caminho Completo", width=400)
    ...
    def create_duplicates_table(self, parent_tab):
        frame = ttk.Frame(parent_tab)
        frame.pack(fill='both', expand=True, padx=5, pady=5)
        cols = ("Ficheiro / Grupo", "Tamanho (MB)")
        self.duplicates_tree = ttk.Treeview(frame, columns=cols, show='headings')
        self.duplicates_tree.heading("Ficheiro / Grupo", text="Ficheiro / Grupo")
        self.duplicates_tree.heading("Tamanho (MB)", text="Tamanho (MB)")
        self.duplicates_tree.column("Tamanho (MB)", anchor='e')
        ...
        self.btn_delete_duplicates = ttk.Button(btn_frame, text="Apagar Selecionados", command=self.delete_selected_duplicates, state='disabled')
        self.btn_delete_duplicates.pack(side='left', pady=5)
        self.btn_export = ttk.Button(btn_frame, text="Exportar Resultados", command=self.export_to_excel, state='disabled')
        self.btn_export.pack(side='right', pady=5)
        
    def create_old_files_table(self, parent_tab):
        frame = ttk.Frame(parent_tab)
        frame.pack(fill='both', expand=True, padx=5, pady=5)
        cols = ("Ficheiro", "Tamanho (MB)", "Último Acesso")
        self.old_files_tree = ttk.Treeview(frame, columns=cols, show='headings')
        for col in cols:
            self.old_files_tree.heading(col, text=col)
        self.old_files_tree.column("Ficheiro", width=500)
        self.old_files_tree.column("Tamanho (MB)", anchor='e', width=120)
        self.old_files_tree.column("Último Acesso", anchor='center', width=150)
        ...
        btn_frame = ttk.Frame(parent_tab)
        btn_frame.pack(fill='x', padx=5)
        self.btn_compress_old_files = ttk.Button(btn_frame, text="Comprimir Selecionados para .zip", command=self.compress_selected_old_files, state='disabled')
        self.btn_compress_old_files.pack(side='left', pady=5)
        
    def create_context_menu(self):
        self.context_menu = tk.Menu(self, tearoff=0)
        self.context_menu.add_command(label="Abrir Localização", command=self.open_file_location)
        self.context_menu.add_separator()
        self.context_menu.add_command(label="Abrir Ficheiro", command=self.open_file)
    ...
    def show_context_menu(self, event):
        ...
        if item_id:
            ...
            if os.path.isdir(item_path):
                self.context_menu.entryconfig("Abrir Ficheiro", state="disabled")
            else:
                self.context_menu.entryconfig("Abrir Ficheiro", state="normal")
    ...
    def show_nav_context_menu(self, event):
        ...
        if item_id:
            ...
            self.context_menu.entryconfig("Abrir Ficheiro", state="disabled")
            self.context_menu.entryconfig("Abrir Localização", label="Abrir Pasta")
            self.context_menu.post(event.x_root, event.y_root)
    ...
    def reset_view_state(self):
        ...
        self.lbl_status.config(text="Selecione uma pasta à esquerda para analisar.")
        self.lbl_status.pack(pady=50)
        self.clear_filters()
    ...
    def apply_filters(self):
        ...
        try:
            ...
            messagebox.showerror("Erro de Valor", "Insira apenas números nos campos de tamanho.")
            return
        ...
        try:
            with pd.ExcelWriter(path) as writer:
                ...
                if not all_content.empty:
                    all_content.to_excel(writer, sheet_name="Conteúdo da Pasta", index=False)
                if self.duplicate_groups:
                    pd.DataFrame(self.duplicate_groups).to_excel(writer, sheet_name="Ficheiros Duplicados", index=False)
                if self.old_files:
                    pd.DataFrame(self.old_files).to_excel(writer, sheet_name="Ficheiros Antigos", index=False)
    ...
    def compress_selected_old_files(self):
        ...
        if not selected_iids:
            messagebox.showwarning("Nenhum Ficheiro", "Selecione os ficheiros a comprimir.")
            return
        ...
        try:
            ...
            messagebox.showerror("Caminho Inválido", "O diretório para salvar o arquivo não existe.")
            return
            
        if not messagebox.askyesno("Confirmar Arquivamento", f"Comprimir {len(selected_iids)} ficheiro(s) e APAGAR os originais?"):
            return
    ...
    def get_status_label(self):
        try:
            ...
            if current_tab_text == "Duplicados":
                return ttk.Label(self.duplicates_tab)
            elif current_tab_text == "Ficheiros Antigos":
    ...
    def set_ui_busy(self, is_busy: bool) -> None:
        ...
        self.btn_find_duplicates.config(state=state)
        self.btn_find_old_files.config(state=state)
    ...
    def update_quick_analysis_view(self):
        ...
        all_content = pd.concat([self.df_folders, self.df_files], ignore_index=True)
        if all_content.empty:
            self.lbl_status.config(text="Pasta vazia ou sem conteúdo acessível.")
    ...
    def update_duplicates_view(self):
        ...
        if self.duplicate_groups:
            ...
            messagebox.showinfo("Duplicados Encontrados", f"Foram encontrados {len(self.duplicate_groups)} grupos de ficheiros duplicados.")
        else:
            messagebox.showinfo("Nenhum Duplicado", "Não foram encontrados ficheiros duplicados nesta pasta.")
