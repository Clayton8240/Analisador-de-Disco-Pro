...
class FinalDiskAnalyzerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Analisador de Disco Pro - Versão Final")
    ...
    def create_view_panel(self):
        """Cria e organiza todos os widgets do painel direito."""
        action_frame = ttk.Frame(self.view_frame)
        action_frame.pack(fill='x', pady=5)
        ttk.Label(action_frame, text="Pasta Selecionada:").pack(side='left')
        path_label = ttk.Label(action_frame, textvariable=self.current_path, font=('Arial', 10, 'italic'), relief='sunken', padding=5)
        ...
        self.lbl_status = ttk.Label(self.chart_tab, text="Selecione uma pasta à esquerda para iniciar a análise.", font=('Arial', 12))
    ...
    def create_file_list_table(self, parent_tab):
        ...
        v_scroll = ttk.Scrollbar(frame, orient="vertical", command=self.files_tree.yview); h_scroll = ttk.Scrollbar(frame, orient="horizontal", command=self.files_tree.xview)
        ...
        # NOVO: Associa o evento de duplo-clique
        self.files_tree.bind("<Double-1>", self.on_double_click_item)
        
    def on_double_click_item(self, event):
        """Chamado ao dar duplo-clique num item da lista detalhada."""
        # A lógica de abrir a localização já existe, apenas a reutilizamos.
        self.open_file_location()
        
    # Restante do código (com a aba de "Tipos" e suas referências removidas)
    ...
    def update_pie_chart(self):
        ...
        df_plot = chart_data.nlargest(10, 'size')
        if len(chart_data) > 10:
            outros_size = chart_data.iloc[10:]['size'].sum(); outros_row = pd.DataFrame([{'name': 'Outros', 'size': outros_size}])
            df_plot = pd.concat([df_plot, outros_row], ignore_index=True)
        fig = plt.Figure(figsize=(6, 5), dpi=100); ax = fig.add_subplot(111)
        ax.pie(df_plot['size'], labels=df_plot['name'], autopct='%1.1f%%', startangle=90, textprops={'fontsize': 9})
        ax.axis('equal'); fig.tight_layout()
        self.fig_canvas = FigureCanvasTkAgg(fig, master=self.chart_tab); self.fig_canvas.draw(); self.fig_canvas.get_tk_widget().pack(fill='both', expand=True)
